...
def create_campaign(db_user_creator, db_campaign, name, notes, configurations, start_timestamp, end_timestamp, remove_inactive_users_timeout):
    cur = get_db_connection().cursor(cursor_factory=psycopg2_extras.DictCursor)
    ...
    elif db_campaign['creator_id'] == db_user_creator['id']:
        cur.execute('update "et"."campaign" set "creator_id" = %s, "name" = %s, "notes" = %s, "config_json" = %s, "start_timestamp" = %s, "end_timestamp" = %s, "remove_inactive_users_timeout" = %s where "id"=%s;', (
            db_user_creator['id'],
            name,
            notes,
            configurations,
...
def store_data_source(db_creator_user, name, icon_name):
    cur = get_db_connection().cursor(cursor_factory=psycopg2_extras.DictCursor)
    cur.execute('insert into "et"."data_source"("creator_id", "name", "icon_name") values (%s,%s,%s);', (
        db_creator_user['id'],
        name,
        icon_name
    ))
